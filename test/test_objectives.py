...
from pychop3d import bsp_tree, settings, objective_functions
...
def test_number_of_parts():
    ...
    tree, result = bsp_tree.expand_node(tree, tree.nodes[0].path, Plane(np.zeros(3), np.array([0, 0, 1])))
    assert tree.objectives['nparts'] == 1
...
def test_utilization():
    ...
    mesh = trimesh.primitives.Box(extents=(100., 100., 220.)).to_mesh()
    tree1 = bsp_tree.BSPTree(mesh, printer_extents)
    tree1, result = bsp_tree.expand_node(tree1, tree1.nodes[0].path, Plane(np.zeros(3), np.array([0., 0., 1.])))
    tree2 = bsp_tree.BSPTree(mesh, printer_extents)
    tree2, result = bsp_tree.expand_node(tree2, tree2.nodes[0].path, Plane(np.array([0., 0., 100.]), np.array([0., 0., 1.])))
    objective_functions.evaluate_utilization_objective([tree1, tree2], (), printer_extents)
